...
rawdat <- dat %>%
  ...
  dplyr::mutate(temp_status = case_when(temperature > 27 ~ "NP",
                                        temperature <= 27 ~ "P"
                                        )) %>%
...
# Write to file
write_csv(final_df, "output/residue_extraction/channelAB_categorical_10_loops_2_groups_27_cutoff_0.7_split.csv")
